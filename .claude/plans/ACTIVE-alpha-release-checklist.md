...
- [ ] Stale session detection (>24hr in ready state) displays correctly
- [ ] State change animations (flashing/pulsing indicators) are smooth
- [ ] Ready chime plays correctly when a session becomes ready
...
- [ ] **Hide/remove other terminals from UI** — Alacritty, kitty, Warp, IDE terminals should not appear as options in alpha (or at minimum, mark as unsupported)
- [ ] Terminal activation gracefully fails if target terminal isn't running

### Project Management
- [ ] **Browse to add** — "Link Project" via `NSOpenPanel` works, validates project directory
- [ ] **Drag-and-drop to add** — Drop folder onto app window to add project
- [ ] **Remove project** — "Remove from HUD" action works cleanly
- [ ] **Project validation** — Rejects invalid paths, handles edge cases (already tracked, dangerous paths, suggest parent)
...
- [ ] Order persists across app restarts (UserDefaults)
- [ ] Reordering only applies to active (non-paused) projects

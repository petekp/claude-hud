...
Tests use in-memory backup/restore to avoid corrupting the real `~/.claude/hud-session-states.json`. The original state is captured before tests and restored after, even if tests fail.
